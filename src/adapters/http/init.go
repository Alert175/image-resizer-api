...
import (
	"fmt"
	"image-resizer-api/src/adapters/http/control"
	"log"
	"os"
...
func Init() {
	...
	// настройка адаптера
	appPathPrefix := os.Getenv("APP_URL_PREFIX")
	appPort := os.Getenv("APP_PORT")
	if len(appPathPrefix) == 0 || len(appPort) == 0 {
		log.Fatal("http env not found")
	}
	configLogger(app)
	configCors(app)

	appPath := app.Group(appPathPrefix)

	appPath.Get("/swagger/*", swagger.HandlerDefault)

	control.Register(appPath)
